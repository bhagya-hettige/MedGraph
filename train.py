...
def main():
    ...
    parser.add_argument('name', default='mimic')
...
def train(args):
    ...
    graph_file = '/Users/bhagya/PycharmProjects/MedGraph/data/%s/%s.npz' % (args.name, args.name)
    ...
    # data_loader.vv_batch_size = args.vv_batch_size
    ...
    global_step = 0
    num_tr_iter = len(data_loader.vv_train) // args.batch_size
    print('Number of iterations per epoch: {}'.format(num_tr_iter))
    ...
    with tf.Session() as sess:
        ...
        for epoch in range(epochs):
            ...
            for iteration in range(num_tr_iter):
                ...
                end = (iteration + 1) * args.batch_size if iteration < num_tr_iter else data[0].shape[0]
                ...
                (vc_u_i, vc_u_j, vc_label) = data_loader.fetch_vc_batch(batch_size=args.batch_size_vc, K=args.K)
            ...
            print('---------------------------------------------------------')
            print("Epoch: {0}, validation auc: {1:.01%}, validation ap: {2:.01%}, "
                  "test auc: {3:.4f}, test ap: {4:.4f}, test precision: {5:.4f}, test recall: {6:.4f}".
                  format(epoch + 1, val_auc, val_ap, test_auc, test_ap, test_precision, test_recall))
            print('---------------------------------------------------------')
